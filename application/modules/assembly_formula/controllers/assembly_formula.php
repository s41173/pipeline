...
class Assembly_formula extends MX_Controller
{
    function __construct()
    {
        parent::__construct();
        
        $this->load->model('Assembly_formula_model', 'model', TRUE);
        $this->load->model('Assembly_formula_item_model', 'transmodel', TRUE);
        $this->load->model('Assembly_formula_cost_model', 'costmodel', TRUE);
	...
	{ 
           $profit = intval($this->product->get_price($res->product)-$res->amount);
	   $output[] = array ($res->id, tglin($res->dates), $res->docno, $res->currency, $this->branch->get_name($res->branch_id),$res->approved, $res->log,
           strtoupper($this->product->get_name($res->product)), $res->qty, $res->amount, 'ASM-0'.$res->id, $this->product->get_price($res->product),$profit, intval($profit/100),
           $this->product->get_category($res->product)                             );
	}
            $this->output
	...
	$data['form_action'] = site_url($this->title.'/add_process');
        ...
        $data['productcombo'] = $this->transmodel->combo();
	// ---------------------------------------- //
        ...
        $this->table->set_heading('#', 'No', 'Code','Date', 'Code', 'HPP', 'M-Pro', 'M-Pro(%)', 'Product - Qty', 'Category', 'Action');
...
//            $this->db->trans_complete();
           ...
           if ($this->db->trans_status() === FALSE){ echo "error|ASMF-00$stock_adjustment->no failed confirmed..!";  }
           else { echo "true|ASMF-0$pid confirmed..!"; }
        }
...
//    ===================== approval ===========================================
    ...
    function calculate(){
        ...
        $hasil = TRUE;
        foreach ($result as $res) {
            $total = $this->transmodel->total($res->id);
            $amt = intval($total['total'])+intval($res->costs);
            $stock_adjustment = array('amount' => $amt);
            if ($this->model->update($res->id, $stock_adjustment) == FALSE){break; $hasil = FALSE;}
        }
        if ($hasil == TRUE){ echo 'true|Calculate Successed..!'; }else{ echo 'error|Calculate Failure...!'; }
    }
    
    function set_adjustment(){
        $hasil = TRUE;
        if (!$this->input->post('cproduct')){$hasil = FALSE;}
        if ($hasil == TRUE){
            $price = $this->product->get_price($this->input->post('cproduct'));
            $result = $this->transmodel->get_item_by_product($this->input->post('cproduct'))->result();
            foreach ($result as $res) {
              if ($this->transmodel->update($res->id,intval($res->qty*$price)) == FALSE){ break; $hasil = FALSE; }
            }
        }
        if ($hasil == TRUE){$this->calculate(); }else{ echo 'error|Adjustment Failure...!'; }
    }
...
//       }
	...
	// Form validation
        ...
        $this->form_validation->set_rules('coutlet', 'Outlet Storage', 'required');
        $this->form_validation->set_rules('tprodcost', 'Production Cost', 'required|numeric');
        
        $this->form_validation->set_rules('titemstockacc', 'Stock-Account', 'required');
        $this->form_validation->set_rules('titemcostacc', 'Cost-Account', 'required');
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            $stock_adjustment = array('docno' => $this->input->post('tdocno'), 'product' => $this->product->get_id_by_sku($this->input->post('tproduct')), 'qty' => $this->input->post('tqty'), 
                                      'currency' => $this->input->post('ccurrency'), 'dates' => $this->input->post('tdate'), 'branch_id' => $this->input->post('coutlet'),
                                      'notes' => $this->input->post('tnote'), 'tax' => $this->input->post('ctax'), 'project' => setnull($this->input->post('tproject')),
                                      'account' => $this->input->post('cacc'), 'formula' => 1, 'prodcost' => $this->input->post('tprodcost'),
                                      'stock_acc' => $this->account->get_id_code($this->input->post('titemstockacc')), 'cost_acc' => $this->account->get_id_code($this->input->post('titemcostacc')),
	...
	$data['form_action'] = site_url($this->title.'/update_process/'.$id);
        ...
        $data['default']['prodcost'] = intval($cash->prodcost);
        $data['default']['outlet'] = $cash->branch_id;
        
        $data['default']['stockacc'] = $this->account->get_code($cash->stock_acc);
        $data['default']['costacc'] = $this->account->get_code($cash->cost_acc);
...
//        $this->form_validation->set_rules('tamount', 'Unit Price', 'required|numeric|is_natural_no_zero');
        ...
        if ($this->form_validation->run($this) == TRUE && $this->valid_confirmation($pid) == TRUE)
        {
            ...
            $this->db->trans_start();
            $id = $this->transmodel->counter();
            $price = $this->product->get_detail_based_sku($this->input->post('tproduct'));
...
//                                   $qty, $pid, 'ASM', $id);
            ...
            $pitem = array('id' => $id, 'product_id' => $this->product->get_id_by_sku($this->input->post('tproduct')), 'assembly' => $pid,
                           'qty' => $qty, 'price' => intval($qty*$price->pricelow));
...
//    ==========================================================================================
	...
	// Form validation
        ...
        $this->form_validation->set_rules('tid', 'Transcode', 'required|callback_valid_confirmation');
        ...
        {   
            $stock_adjustment = array('docno' => $this->input->post('tdocno'),'product' => $this->product->get_id_by_sku($this->input->post('tproduct')), 'qty' => $this->input->post('tqty'), 
                                      'currency' => $this->input->post('ccurrency'), 'dates' => $this->input->post('tdate'), 'branch_id' => $this->input->post('coutlet'),
                                      'account' => $this->input->post('cacc'), 'prodcost' => $this->input->post('tprodcost'),
                                      'notes' => $this->input->post('tnote'),'tax' => $this->input->post('ctax'),'project' => setnull($this->input->post('tproject')),
                                      'stock_acc' => $this->account->get_id_code($this->input->post('titemstockacc')), 'cost_acc' => $this->account->get_id_code($this->input->post('titemcostacc')),
    ...
    public function valid_product($pid)
    {
        if ($this->model->valid_product($this->product->get_id_by_sku($pid)) == FALSE){
