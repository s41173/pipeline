...
class Balance extends MX_Controller
{
	...
	$data['form_action'] = site_url($this->title.'/add_process');
        ...
        $ps = $this->period->get();
        $data['startmonth'] = $ps->start_month;
        $data['startyear'] = $ps->start_year;
	// ---------------------------------------- //
    ...
    function reset_balance(){
        if ($this->acl->otentikasi_admin($this->title,'ajax') == TRUE){
            ...
            if ( $ps->month == $ps->start_month && $ps->year == $ps->start_year ){
                if ($this->balancelib->reset() == true){ echo 'true|Balance Reset Processed';
                }else{ echo 'warning|Balance Reset Unsuccessull'; }
            }else{ echo 'error|Invalid Begin Period'; }
    ...
    function update($uid)
    {
        ...
        echo $uid.'|'.$acc_lib->get_code($uid).'|'.$acc_lib->get_name($uid).'|'.$this->get_balance($uid);
    }
	...
	// Form validation
        $this->form_validation->set_rules('tbalance', 'Balance', 'required|numeric|callback_valid_setting['.$this->input->post('tid').']');
        ...
        {       
            ...
            $bl->where('account_id', $this->input->post('tid'));
            $bl->where('month', $ps->month);
            $bl->where('year', $ps->year)->get();
                        
            $bl->beginning = $this->input->post('tbalance');
            $bl->vamount = $this->journal->calculate_account_amount($this->input->post('tid'), $this->input->post('tbalance'));
