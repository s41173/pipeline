    ...
    <div class="table-responsive">
      <table class="table table-striped jambo_table bulk_action">
        ...
        <?php
            ...
            {
                $acc = new Product_lib();
                if ($type == 'name'){ return strtoupper($acc->get_name($val)); }
                elseif ($type == 'unit'){return $acc->get_unit($val); }
                elseif ($type == 'sku'){ return $acc->get_sku($val); }
                elseif ($type == 'model'){ return $acc->get_name($val,'model'); }
            }
                ...
                foreach($items as $res)
                {
                    echo "
                     <tr class=\"even pointer\">
                        <td> ".$i." </td>
<td>".product($res->product,'sku').'<br>'.product($res->product,'name').' - '.product($res->product,'model')." </td>
