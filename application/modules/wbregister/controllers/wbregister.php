...
class Wbregister extends MX_Controller
{
    function __construct()
    {
        parent::__construct();
        
        $this->load->model('Wbregister_model', '', TRUE);
    ...
    {
        if(!$search){ $result = $this->Wbregister_model->get_last_category($this->modul['limit'])->result(); }
        
        if ($result){
	foreach($result as $res)
	{
           $merged = '-';
           if ($res->merge_contract != null){$merged = $res->merge_contract;}
	   $output[] = array ($res->id, $res->contract_no, $res->demand_qty,$merged);
	}
            $this->output
    ...
    function publish($uid = null)
    {
       ...
       $val = $this->Wbregister_model->get_by_id($uid)->row();
       if ($val->publish == 0){ $lng = array('publish' => 1); }else { $lng = array('publish' => 0); }
       $this->Wbregister_model->update($uid,$lng);
    ...
    function get_last_category()
    {
        ...
        $data['main_view'] = 'wbregister_view';
	...
	// ---------------------------------------- //
        ...
        $this->table->set_heading('#','No', 'Contract-No', 'Demand-Qty', 'Contract Merged', 'Action');
    ...
    function delete_all()
    {
      ...
      if($cek)
      {
        ...
        for ($i=0; $i<$jumlah; $i++)
        {
           ...
           {
              $img = $this->Wbregister_model->get_by_id($cek[$i])->row();
              $img = $img->image;
              if ($img){ $img = "./images/category/".$img; unlink("$img"); }

              $this->Wbregister_model->delete($cek[$i]); 
           }
           else { $x=$x+1; }
    ...
    function delete($uid,$type='soft')
    {
        if ($this->acl->otentikasi_admin($this->title,'ajax') == TRUE){
        if ($type == 'soft'){
           $this->Wbregister_model->delete($uid);
       ...
       else
       {
        ...
        {
           $img = $this->Wbregister_model->get_by_id($uid)->row();
           $img = $img->image;
           if ($img){ $img = "./images/category/".$img; unlink("$img"); }
...
//
            ...
            $this->Wbregister_model->add($category);
...
//            redirect($this->title);
    ...
    {        
        ...
        $category = $this->Wbregister_model->get_by_id($uid)->row();
...
//        $this->load->view('category_update', $data);
    ...
    {
        if ($this->Wbregister_model->valid('name',$name) == FALSE)
        {
    ...
    {
        if ($this->Wbregister_model->valid('code',$name) == FALSE)
        {
	...
	if ($this->Wbregister_model->validating('name',$name,$id) == FALSE)
        {
    ...
    {
	$id = $this->session->userdata('langid');
	if ($this->Wbregister_model->validating('code',$name,$id) == FALSE)
        {
	...
	// Form validation
	    ...
	    $this->Wbregister_model->update($this->session->userdata('langid'), $category);
    ...
    // ====================================== CLOSING ======================================
    function reset_process(){ $this->Wbregister_model->closing(); } 
