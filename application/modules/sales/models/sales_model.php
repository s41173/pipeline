...
class Sales_model extends Custom_Model
{
    ...
    protected $field = array('id', 'dates', 'contract_id', 'cust_id', 'amount', 'tax', 'cost', 'total', 'shipping', 'branch_id',
                             ...
                             'approved', 'cash', 'log', 'created', 'updated', 'deleted', 'discount_desc');
    protected $com;
...
//        $this->cek_null(, 'confirmation');
    ...
    function total_summary($branch=null, $cust=null, $start=null,$end=null,$paid=null,$confirm=null){
    ...
    {   
        $this->db->select('sales.id, sales.branch_id, sales.dates, log.userid, product.id as pid, product.sku, product.name, sales_item.orderid, sales_item.qty, sales_item.price, sales.confirmation, '
                . '        category.name as category, manufacture.name as manufacture');
        $this->db->from('sales, sales_item, product, category, manufacture, log');
        $this->db->where('sales.id = sales_item.sales_id');
        $this->db->where('sales.log = log.id');
        $this->db->where('sales_item.product_id = product.id');
        $this->db->where('product.category = category.id');
