...
class Log extends MX_Controller
{
    ...
    public function getdatatable($search=null,$logid='null',$user='null',$activity='null',$modul='null',$date='null')
    {
        if(!$search){ $result = $this->model->get_last_user($this->modul['limit'])->result(); }
        else{
            if ($logid != 'null'){ 
                $result = $this->model->search($logid,'null','null','null','null')->result(); 
                
            }
            else{
               $result = $this->model->search($logid,$user,$activity,$modul,$date)->result(); 
            }
        }
	 ...
	 {
	   $output[] = array ($res->id, $this->user->get_username($res->userid), tglin($res->date), $res->time, 
                              $this->com->get_name($res->component_id), $res->activity, $res->field, $res->description, $res->prev_val,
	...
	$data['form_action'] = site_url($this->title.'/add_process');
        ...
        $data['user'] = $this->model->combo_user();
        $data['activity'] = $this->model->combo_activity();
	...
	// ---------------------------------------- //
        ...
        $this->table->set_heading('#', 'No', 'Log-ID', 'Username', 'Date', 'Time', 'Component', 'Activity', 'Field', 'Action');
...
//            echo validation_errors();
   ...
   function invoice($pid)
   {
        ...
        $data['title'] = $this->properti['name'].' | Administrator '.ucwords($this->modul['title']);
        $data['dates'] = tglin($journal->date);
        $data['time'] = $journal->time;
        $data['username'] = $this->user->get_username($journal->userid);

        $data['uid'] = $pid;
        $data['modul'] = $this->com->get_name($journal->component_id);
        $data['activity'] = $journal->activity;
        $data['field'] = $journal->field;
        $data['prev'] = json_decode($journal->prev_val);
        $data['desc'] = json_decode($journal->description);
        
        $this->load->view('log_invoice', $data);
   }
