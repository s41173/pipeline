...
class Log_model extends Custom_Model
{
    ...
    function search($logid=null,$user=null,$activity=null,$modul=null,$date=null)
    {   
        $this->db->select($this->field);
        $this->db->from($this->tableName); 
        $this->db->where('deleted', $this->deleted);
        $this->cek_null_string($logid, 'id');
        $this->cek_null_string($date, 'date'); 
        $this->cek_null_string($user, 'userid');
        $this->cek_null_string($activity, 'activity');
        $this->cek_null_string($modul, 'component_id');
        $this->db->order_by('id', 'desc'); 
        return $this->db->get(); 
    }
