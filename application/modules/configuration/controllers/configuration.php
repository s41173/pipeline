...
//        $this->acl->otentikasi();
    ...
    function setotp($otp=0,$pin=0){
        
       if ($this->acl->otentikasi_admin($this->title,'ajax') == TRUE){ 
        
        if ($pin == intval(date('dmY'))){
           $property = array('token_url' => intval($otp));
           $this->Configuration_model->update(1, $property);
           echo "true| OTP has successfully updated..! "; 
        }else{ echo 'error|Invalid Pin.'; }
       }
    ...
    public function getdatatable($search=null)
    {
        ...
        $output[] = array ($result->id, $result->name, $result->address, $result->phone1, $result->phone2, $result->email,
                              ...
                              $ps->start_year, $ps->closing_month, $result->token_url
                             );
	...
	// Form validation
	   ...
	   $this->form_validation->set_rules('ccity', 'City', 'max_length[25]');
