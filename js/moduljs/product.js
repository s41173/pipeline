$(document).ready(function (e) {
	...
	{	
		...
		var element = $(this);
		// var del_id = element.attr("id");
		var res = element.attr("id").split("|");
		var url = sites_ledger +"/"+ res[0]+"/"+res[1];
	...
	$('#searchform').submit(function() {
		...
		var sku = $("#tsku").val();
		var type = $("#ctype").val();
		var param = ['searching',branch,cat,color,size,publish,sku,type];
...
// document ready end	
	...
	function load_data_search(search=null)
	{
		$(document).ready(function (e) {
		    ...
		    $.ajax({
				type : 'GET',
				url: source+"/"+search[0]+"/"+search[1]+"/"+search[2]+"/"+search[3]+"/"+search[4]+"/"+search[5]+"/"+search[6]+"/"+search[7],
...
'<a href="" class="btn btn-warning btn-xs text-ledger" id="' +s[i][0]+"|"+s[i][14]+ '" title=""> <i class="fa fas-2x fa-book"> </i> </a> '+
