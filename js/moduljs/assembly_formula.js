$(document).ready(function (e) {
	...
	{	
		e.preventDefault();
		var value = $("#titems2").val();
		var url = sites+"/get_unit_price/"+value;
		if (value){
			// batas
			$.ajax({
				type: 'POST',
	...
	$(document).on('click','.text-remove-cost',function(e)
	{	
		e.preventDefault();
		var element = $(this);
		var del_id = element.attr("id");
		var url = sites +"/delete_cost/"+ del_id;
		$(".error").fadeOut();
	...
	// prodcost
	$(document).on('keyup','#tprodcost',function(e)
	{	
		e.preventDefault();
	...
	$('#ajaxtransform,#ajaxtransform2').submit(function() {
